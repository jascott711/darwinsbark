...
function slideShow() {
  ...
  var globals = {
    ...
    buttonStartText: "", // Text used in the slide show toggle button.
    buttonStopText: "", // Text used in the slide show toggle button.    
    ...
    nextButtonID: "nextSlide",

    prevButtonObject: null,
    prevButtonID: "prevSlide"
  }
  ...
  globals.nextButtonObject.addEventListener('click', nextSlide, false); // This callback is used to toggle the slide show on and off.
  globals.prevButtonObject.addEventListener('click', prevSlide, false); // This callback is used to toggle the slide show on and off.
  ...
  function initializeGlobals() {   
    ...
    globals.nextButtonObject = (document.getElementById(globals.nextButtonID) ? document.getElementById(globals.nextButtonID) : null);   
    globals.prevButtonObject = (document.getElementById(globals.prevButtonID) ? document.getElementById(globals.prevButtonID) : null);  
  ...
  function initializeSlideShowMarkup() {  
    ...
    globals.wrapperObject.style.width = 100 + "%";
    globals.wrapperObject.style.height = slideHeightMax - 5 + "px";
    ...
    for (var i = 0; i < slideCount; i++) { 
      ...
      globals.slideImages[i].style.width = 100 + "%";                 
    }
  ...
  function toggleSlideShow() {
    if (globals.slideShowRunning) {
      ...
      if (globals.buttonObject) { 
        ...
        //globals.buttonObject.textContent = '<i class="fa fa-play"></i>'; 
      }
    }
    else {
      startSlideShow();
      if (globals.buttonObject) { 
        ...
        //globals.buttonObject.textContent = '<i class="fa fa-pause"></i>'; 
      }            
    }
  ...
  function prevSlide() {
    var currentSlide = globals.slideImages[globals.slideIndex];
    console.log(globals.slideImages[globals.slideIndex]);
    
    console.log(globals.slideIndex);
    ...
    var prevSlide = globals.slideImages[globals.slideIndex];

    console.log(globals.slideImages[globals.slideIndex]);
    
    var currentSlideOpacity = 1; // Fade the current slide out.
    var prevSlideOpacity = 0; // Fade the next slide in.
    var opacityLevelIncrement = 1 / globals.fadeDelay;
    var fadeActiveSlidesID = setInterval(fadeActiveSlides, globals.fadeDelay);
    
    function fadeActiveSlides() {
      currentSlideOpacity -= opacityLevelIncrement;
      prevSlideOpacity += opacityLevelIncrement;
      
      console.log(currentSlideOpacity + prevSlideOpacity); // This should always be very close to 1.
      
      if (currentSlideOpacity >= 0 && prevSlideOpacity <= 1) {
        currentSlide.style.opacity = currentSlideOpacity;
